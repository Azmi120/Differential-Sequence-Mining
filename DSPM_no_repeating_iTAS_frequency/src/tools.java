...
public class tools {
	//Tasmia code starts
	List<String> iTASList;
	int length_iTAS = -1;
	public void setiTAS(List<String> iTASList) {
		this.iTASList = iTASList;
		length_iTAS = iTASList.size();
	}
	//Tasmia code starts
	...
	public void countPattern(List<Student> students, List<String> patterns,int gap){
		...
		for(Student stu:students){
			...
			if(stu.id.equals("Stu_503f097e70fb39d08a784f3e21a79340")) {
			...
			if(stu.has_init == 0) {
				...
				iTAS_freq = new String[length_iTAS];
				Arrays.fill(iTAS_freq,new String("0"));
				iTAS_freq[0] = stu.id;
			}
			else {
				iTAS_freq = stu.getiTASFreq();
			}
			// Tasmia edit ends:
			...
			for(int j=0;j<patterns.size();j++){
				...
				// Tasmia edit starts:
				if(iTASList.contains(patterns.get(j))){
					iTAS_freq[iTASList.indexOf(patterns.get(j))] = stu.patternFre.get(patterns.get(j))+"";
				}
				// Tasmia edit ends:
			}
			// Tasmia edit starts:
			stu.setiTASFreq(iTAS_freq);
			// Tasmia edit ends:
		}
	}
	
	public void myCountingPattern(Student stu, String pattern,int gap){
		/*if(pattern.equals("QSQMT")) {
		...
		for(int start=0;start<stu.activity.length()-pattern.length();start++){
			/*if(start>150) {
				System.out.println(stu.patternFre.get("QSQMT"));
			}*/
	...
	public void T_Test(List<patternTTest> patterns_result,List<Student> students,double pThreshold, int goodnum, int badnum){
		System.out.println("Couducting t-test for each patterns..."+patterns_result.toString());

		TTest tt=new TTest();
		for(int i=0;i<patterns_result.size();i++){
			
			//int goodnum=86,badnum=86;
			...
			int goodp=0,badp=0;
			for(int j=0;j<students.size();j++){
				if(students.get(j).class1.equals("high-gainer")){
				...
				if(students.get(j).class1.equals("low-gainer"
						+ ""
						+ "")){
			...
			System.out.println("p value of "+patterns_result.get(i).pattern+" : " +patterns_result.get(i).p_value);
		...
		for(int i=0;i<patterns_result.size();i++){
			if(patterns_result.get(i).p_value<pThreshold) {
				System.out.println("p value of < threshold "+patterns_result.get(i).pattern+" : " +patterns_result.get(i).p_value+" t value: "+patterns_result.get(i).tStat);
				continue;
			}
			else 
